...
class Multicolour_Route_Templates {
  ...
  static GET(request, reply) {
    ...
    query.exec((err, models) =>
      err ? Multicolour_Route_Templates.error_reply(err, reply) : reply(models).code(200))
  }
  ...
  static PUT(request, reply) {
    this.update(request.params.id, request.payload, (err, model) =>
      err ? Multicolour_Route_Templates.error_reply(err, reply) : reply(model).code(202))
  }
  ...
  static UPLOAD(request, reply) {
    ...
    this.findOne({
      ...
      // Check we found models.
        ...
        reply(Boom.notFound("Pending upload failed, could not find name_key"))
      }
      // Upload the file.
      else {
        // Upload the file.
        host.request("storage")
          .upload(request.payload.file.path, {
            name: `${model.name_key}${require("path").extname(request.payload.file.filename)}`
          })
