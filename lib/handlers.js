...
class Multicolour_Route_Templates {
    ...
    this.create(request.payload, (err, model) => {
      if (err) {
        ...
        this.findOne(model)
          ...
          // Execute the query.
          .exec((err, models) => {
            err ? Multicolour_Route_Templates.error_reply(err, reply, this) : reply[host.request("decorator")](models, this).code(202)
          })
      }
    })
  }
  ...
  static DELETE(request, reply) {
    this.destroy(request.params.id, (err, model) =>
      err ? Multicolour_Route_Templates.error_reply(err, reply, this) : reply[host.request("decorator")](model, this).code(202))
  }
  ...
  static UPLOAD(request, reply) {
    ...
    this.findOne({
      ...
      // Upload the file.
        ...
        const name = `${this.file_path || ""}/${model.id}${extension}`
