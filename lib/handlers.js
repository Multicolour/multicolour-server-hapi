...
class Route_Templates {
  ...
  get_decorator_for_apply_value(reply_interface, accept_value) {
    ...
    // Return the function.
    if (accept && reply_interface.hasOwnProperty(accept))
      return reply_interface[accept].bind(reply_interface)
    else {
      this.debug(`Could not find a decorator for the Accept header value of '${accept_value}'. Using application/json as default.`)
      return reply_interface["application/json"].bind(reply_interface)
    }
  }
  ...
  check_errors_then_reply(err, decorator, models, collection, code, reply) {
    if (err) {
      ...
      if (err.code === 404)
        return this.get_decorator_for_apply_value(reply, decorator)(boom.notFound("Document(s) not found"), collection)
      else
        return this.get_decorator_for_apply_value(reply, decorator)(err, collection)
    }
    else {
      return this.get_decorator_for_apply_value(reply, decorator)(models, collection).code(code)
    }
  }
