...
class Verb {
  ...
  constructor(verb, model, server) {
    ...
    this.debug = debug(`Multicolour verb ${this._name}@${this._verb}: `)

    this._verb = verb.toUpperCase()
    this._name = model.model_name
    this._model = model
  ...
  get_error_schemas(validators) {
    ...
    validators.forEach(validator => {
      if (!validator.error_schema) return null
    ...
    return Joi.alternatives().try(...valids)
  }
  ...
  get_route() {
    ...
    const headers = this._server.request("header_validator").get()
      ...
      config: {
        ...
        plugins: {
          "hapi-swagger": {
            ...
            payloadType: "json",
            produces: this.get_produces_array(Joi.object(headers))
          }
        },
        validate: {
          headers,
    ...
    let targetOut = Object.assign({}, out)

    // If there's a middleware
    if (this._model.routeConfigMiddleware instanceof Function) {
      targetOut = this._model.routeConfigMiddleware.call(this._model, {
        verb: this,
        routeConfig: targetOut,
      })

      this.debug("Route config middleware finished running. Config before middleware was: ", out)
      this.debug("Route config after middleware was: ", targetOut)
    }

    // Joi up the headers. FFS.
    if (targetOut && targetOut.config && targetOut.config.validate && targetOut.config.validate.headers)
      targetOut.config.validate.headers = Joi.object(targetOut.config.validate.headers).unknown(true)
