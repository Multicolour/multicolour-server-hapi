...
class Verb {
  ...
  get_response_schemas(validators) {
    return validators.map(validator => validator.get_response_schema(this._model))
  }
  ...
  get_payload_schemas(validators) {
    return validators.map(validator => validator.get_payload_schema(this._model))
  }
  ...
  get_route(validators, headers) {
      ...
      config: {
        ...
        plugins: {
          "hapi-swagger": {
            ...
            // https://github.com/glennjones/hapi-swagger/issues/237
            // is closed
            // responses: this.get_response_codes(response_payload, headers),
            payloadType: "json",
        ...
        response: {
          schema: response_alternatives.try.apply(response_alternatives, this.get_response_schemas(validators))
            .meta({ className: this._class_name })
        }
      }
    }
    ...
    if (!~["get", "head"].indexOf(this._verb.toLowerCase())) {
      ...
      out.config.validate.payload = alternatives.try.apply(alternatives, this.get_payload_schemas(validators))
    }
